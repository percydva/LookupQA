...
raw_train = concatenate_datasets(
    [raw_squad['train'], raw_aqa['train'].remove_columns('metadata')])
raw_validation = concatenate_datasets(
    [raw_squad['validation'], raw_aqa['validation'].remove_columns('metadata')])

validation_sets = {'combined': raw_validation}
...
class DocumentReader:
    def __init__(self, checkpoint=None, pretrained_path=None, pretrained_model='bert-base-uncased', train_data=None, validation_data=None, model_out='./reader/model', pred_out='./reader/pred'):
        ...
        self.pipeline = pipeline(
            'question-answering', model=self.model, tokenizer=self.tokenizer, device=0)
        self.max_length = 384
    ...
    def finetune(self, checkpoint):
        self.tokenized_train = self.train_data.map(
            self.preprocess_function, batched=True, remove_columns=self.train_data.column_names)
        self.tokenized_validation = self.validation_data.map(
            self.preprocess_function, batched=True, remove_columns=self.validation_data.column_names)
    ...
    def get_answer(self, question, text):
        ...
        if total_input_length > self.max_length:
            ...
            words = text.split()
            contexts = [' '.join(words[i: min(text_input_length, i+context_length)])
                        for i in range(0, math.ceil(text_input_length/context_length), context_length)]
        else:
            contexts = [text]
        results = []
    ...
    def get_answers(self, data=None, ans_out=None):
        ...
        for example in tqdm(data):
            answers[example['id']] = self.get_answer(
